...
async function loadModel(source,name){
  ...
  document.getElementById('btnCancelLoad').hidden=false;document.getElementById('btnSample').disabled=true;
  ...
  try{
    ...
    if(ticket===loadTicket){document.getElementById('btnCancelLoad').hidden=true;document.getElementById('btnSample').disabled=false;loadController=null;}
  }
}
...
async function loadSample(){
  return loadModel(async signal=>{
    const r=await fetch('sample.psd',{signal});
